...
import {
  ...
  assertFinanceDomainOwner,
  assertGmDomainOwner,
  assertHrDomainOwner,
  assertInventoryDomainOwner,
  assertSuperAdminOwner,
  isCsoDomainOwner,
  isFinanceDomainOwner,
  isGmDomainOwner,
  isHrDomainOwner,
  isInventoryDomainOwner,
  isSuperAdminOwner,
} from './domain-owners';
...
describe('domain owners', () => {
  it('recognises each portal owner and excludes Super Admin from staff domains', () => {
    ...
    expect(isFinanceDomainOwner(user([ROLE_CODES.FINANCE_MANAGER]))).toBe(true);
    expect(isInventoryDomainOwner(user([ROLE_CODES.INVENTORY_MANAGER]))).toBe(true);
    ...
    expect(isFinanceDomainOwner(user([ROLE_CODES.SUPER_ADMIN, ROLE_CODES.FINANCE_MANAGER]))).toBe(
      false,
    );
    expect(isInventoryDomainOwner(user([ROLE_CODES.SUPER_ADMIN, ROLE_CODES.INVENTORY_MANAGER]))).toBe(
      false,
    );
  });
  ...
  it('403s wrong roles on domain asserts', () => {
    ...
    expect(() => assertFinanceDomainOwner(user([ROLE_CODES.HR_MANAGER]))).toThrow(
      expect.objectContaining({ code: API_ERROR_CODES.FORBIDDEN }),
    );
    expect(() => assertInventoryDomainOwner(user([ROLE_CODES.FINANCE_MANAGER]))).toThrow(
      expect.objectContaining({ code: API_ERROR_CODES.FORBIDDEN }),
    );
    ...
    expect(() => assertFinanceDomainOwner(user([ROLE_CODES.FINANCE_MANAGER]))).not.toThrow();
    expect(() => assertInventoryDomainOwner(user([ROLE_CODES.INVENTORY_MANAGER]))).not.toThrow();
