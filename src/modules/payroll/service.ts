...
import {
  ...
  type PayrollAdjustableKey,
} from './calc';
import { letterheadFromCompany } from './letterhead';
import { buildParticulars, loadPayrollInput } from './payroll-input';
...
export function createPayrollService(supabase: SupabaseClient) {
  return {
    ...
    async preview(actor: RequestUser, importId: string) {
      requireManage(actor);
      const loaded = await loadPayrollInput(supabase, importId);
      return {
        importId,
        period: loaded.period.key,
        monthLabel: loaded.period.label,
        calendarDays: loaded.calendarDays,
        employees: loaded.employees,
      };
    },

    async calculate(actor: RequestUser, input: CalculateInput, meta: RequestMeta) {
      requireManage(actor);
      const loaded = await loadPayrollInput(supabase, input.importId);
      const { imp, period, existingRun } = loaded;
      const adjustmentMap = new Map((input.adjustments ?? []).map((row) => [row.employeeId, row]));
      ...
      const { calendarDays, reviews, typeByName, payByEmployee, compByEmployee } = loaded;
      const skipped: { employeeId: string; name: string; reason: string }[] = [];
      const inserts: Record<string, unknown>[] = [];

      for (const emp of loaded.employeeRows) {
        ...
        const baseCompensation = compByEmployee.get(emp.id as string);
        if (!company) {
        ...
        const adjustment = adjustmentMap.get(emp.id as string);
